          ...
          scalafmtOverlay = final: prev: {
            scalafmt = prev.scalafmt.overrideAttrs (
              ...
              let
                ...
                outputHash = "sha256-XkpKmXMvJDS2KpXCEMcjXmwicCV8n+ytesg3aJZp2Es=";
              in
              {
                inherit version;
                passthru = {
        ...
        in
        {
          ...
          packages = rec {
            ...
            scalals = sbt.lib.mkSbtDerivation rec {
              ...
              depsSha256 = "sha256-vF5MuxsFhA64nAJetnEltSYL2wnkiiBdfBcWMzU3Keo=";
          ...
          // (lib.optionalAttrs (system == "x86_64-linux") (
            let
              inherit (pkgs.pkgsCross.aarch64-multiplatform-musl) llvmPackages_18 mkShell;
              llvm-bin = llvmPackages_18.libcxxStdenv.mkDerivation {
                name = "clang-llvm-bin";
                dontUnpack = true;
                dontConfigure = true;
                dontBuild = true;
            ...
            in
            {
              aarch64-cross = mkShell.override { stdenv = llvmPackages_18.libcxxStdenv; } {
                name = "scalals-arm64";
